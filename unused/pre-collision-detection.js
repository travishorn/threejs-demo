...
// Create a floor
var floorGeometry = new THREE.PlaneGeometry(buildingWidth, buildingLength, 10, 10);
var floorMaterial = new THREE.MeshBasicMaterial({ color: 0x00ff00, side: THREE.DoubleSide, wireframe: true });
...
var wall1 = new THREE.Mesh(new THREE.BoxGeometry(buildingWidth, 10, 1), new THREE.MeshBasicMaterial({ color: 0xff0000, wireframe: true }));
wall1.position.set(0, buildingHeight, buildingLength / -2);
walls.push(wall1);

var wall2 = new THREE.Mesh(new THREE.BoxGeometry(buildingLength, 10, 1), new THREE.MeshBasicMaterial({ color: 0x00ff00, wireframe: true }));
wall2.position.set(buildingWidth / 2, buildingHeight, 0);
wall2.rotation.y = Math.PI / 2;
walls.push(wall2);

var wall3 = new THREE.Mesh(new THREE.BoxGeometry(buildingWidth, 10, 1), new THREE.MeshBasicMaterial({ color: 0x0000ff, wireframe: true }));
wall3.position.set(0, buildingHeight, buildingLength / 2);
walls.push(wall3);

var wall4 = new THREE.Mesh(new THREE.BoxGeometry(buildingLength, 10, 1), new THREE.MeshBasicMaterial({ color: 0xffff00, wireframe: true }));
wall4.position.set(buildingWidth / -2, buildingHeight, 0);
wall4.rotation.y = Math.PI / 2;
walls.push(wall4);

walls.forEach((wall) => { console.log(wall.geometry.parameters.depth); scene.add(wall); });
...
function checkCollision() {
  ...
  // If there's an intersection, set the camera position back to where it was
  // before movement tried to occur
